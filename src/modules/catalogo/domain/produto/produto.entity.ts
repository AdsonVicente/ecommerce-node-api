...
class Produto extends Entity<IProduto> implements IProduto {
    ...
    private set categorias(categorias: Array<Categoria>) {
        ...
        const quantidadeCategorias = categorias.length;

        if (quantidadeCategorias < Produto.QTD_MINIMA_CATEGORIAS) {
            throw new ProdutoExceptions.QtdMinimaCategoriasProdutoInvalida();
        }

        if (quantidadeCategorias > Produto.QTD_MAXIMA_CATEGORIAS) {
    ...
    public removerCategoria(categoria: Categoria): Categoria {
        ...
        const quantidadeCategoriasDoProduto: number = this.quantidadeCategorias();

        if (quantidadeCategoriasDoProduto <= Produto.QTD_MINIMA_CATEGORIAS) {
